...
interface MaterialRecipesTableProps {
...
function RecipeCell({ items, highlightItemId, itemLookup }: RecipeCellProps) {
  if (items.length === 0) {
...
function DeviceCell({ deviceId, deviceName, itemLookup }: DeviceCellProps) {
...
export default function MaterialRecipeTable({ recipes, itemLookup, materialItemId }: MaterialRecipesTableProps) {
  if (recipes.length === 0) {
    return null;
  }
  ...
  return (
    <div className="mb-8">
      <h3 className="text-lg font-semibold mb-4">作为原料的配方</h3>
          ...
          <tbody>
            {recipes.map((recipe) => (
              <tr key={recipe.deviceId} className="hover:bg-gray-50">
                ...
                <RecipeCell items={recipe.materials} highlightItemId={materialItemId} itemLookup={itemLookup} />
                <RecipeCell items={recipe.products} itemLookup={itemLookup} />
              </tr>
            ))}
          </tbody>
        </table>
